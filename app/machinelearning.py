...
def pipeline_model(path):
    ...
    # machine results
    mach1inelearning_results = dict( face_detect_score = [] ,
                                     face_name = [] ,
                                     face_name_score = [] ,
                                     emotion_name = [] ,
                                     emotion_name_score = [] ,
    ...
    if len(detections) > 0:
        for i , confidence in enumerate(detections[0,0,:,2]):
            if confidence > 0.5:
                ...
                mach1inelearning_results[ 'count' ].append( count )
                mach1inelearning_results[ 'face_detect_score' ].append( confidence )
                mach1inelearning_results[ 'face_name' ].append( face_name )
                mach1inelearning_results[ 'face_name_score' ].append( face_score )
                mach1inelearning_results[ 'emotion_name' ].append( emotion_name )
                mach1inelearning_results[ 'emotion_name_score' ].append( emotion_score )
                
                count += 1
                
            
    return mach1inelearning_results
